...
class FinalADRModel(nn.Module):
    ...
    def forward(self, edge_index, drug_ids, lab_features):
        ...
        # Handle multiple drugs by averaging their embeddings
        drug_embed = x[drug_ids]
        
        # If multiple drugs, average them to get single embedding
        if drug_embed.dim() > 1 and drug_embed.shape[0] > 1:
            drug_embed = drug_embed.mean(dim=0, keepdim=True)
        elif drug_embed.dim() == 1:
        ...
        if lab_embed.dim() == 1:
            lab_embed = lab_embed.unsqueeze(0)
